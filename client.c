...
int readFile() {
   ...
   if (buffer == NULL) {
      return 0;} //Memory error checking
      ...
      for (;;) {
         ...
         if (whatdo == 1) {
          ...
          //dont send
            ...
            printf("\nSending packet (first 48 bytes):\n");
            printf("Checksum:%d\n", (unsigned int)(unsigned char)currPacket[0]);
            printf("ACK:%c\n", currPacket[1]);
            printf("Sequence #:%c\n", currPacket[2]);
            for (int i = 3; i < 48; i++) {
               printf("%c", currPacket[i]);
            }
         ...
         if (voidPacket[2] == altBit && voidPacket[1] == '1') {
